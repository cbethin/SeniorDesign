""" 
THIS code was entirely ripped off from https://www.kaggle.com/yk1598/symspell-spell-corrector/code who ripped it off from someone else. 
"""
...
class SymSpell:
    ...
    def load_datasets(self, filenames = []):
        ...
        for filename in filenames:
            ...
            all_words += [word.strip() for word in new_words]

        all_words_list = list(set(all_words))
        # with open('external/bad-words.csv') as bf:
        ...
        silence = self.create_dictionary_from_arr(all_words_list, token_pattern=r'.+')
        
        # create a dictionary of rightly spelled words for lookup
        words_dict = {k: 0 for k in all_words_list}
        return words_dict

    def spell_corrector(self, word_list, words_d) -> str:
        result_list = []
        for word in word_list:
            if word not in words_d:
                suggestion = self.best_word(word, silent=True)
...
if __name__ == '__main__':
    ...
    # fetch english words dictionary
    with open('../external/english_words_479k.txt') as f:
        words = f.readlines()
    eng_words = [word.strip() for word in words]
    ...
    sample_text = 'suicial apperciated fcking'
